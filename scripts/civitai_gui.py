...
def git_tag():
    try:
        return subprocess.check_output(
            [os.environ.get("GIT", "git"), "describe", "--tags"],
            shell=False,
            encoding="utf8",
        ).strip()
    except:
...
if not forge:
    try:
        ...
        print(
            "Python module 'packaging' has not been imported correctly, please try to restart or install it manually."
        )
        ver_bool = False
...
def show_multi_buttons(model_list, type_list, version_value):
    ...
    default_subfolder = (
        "Only available if the selected files are of the same model type"
    )
    sub_folders = ["None"]
    BtnDwn = (
        version_value and not version_value.endswith("[Installed]") and not model_list
    )
            ...
            for root, dirs, _ in os.walk(model_folder, followlinks=True):
                if dot_subfolders:
                    ...
                    dirs = [
                        ...
                        if not any(
                            part.startswith(".")
                            for part in os.path.join(root, d).split(os.sep)
                        )
                    ]
                for d in dirs:
    ...
    return (
        gr.Button.update(visible=multi, interactive=multi),  # Download Multi Button
        gr.Button.update(
            visible=BtnDwn
            if multi
            else True
            if not version_value.endswith("[Installed]")
            else False
        ),  # Download Button
        ...
        gr.Dropdown.update(
            visible=multi,
            interactive=multi_file_subfolder,
            choices=sub_folders,
            value=default_subfolder,
        ),  # Selected type sub folder
    )
...
def get_base_models():
    ...
    try:
        options = json_return["error"]["issues"][0]["unionErrors"][0]["issues"][0][
            "options"
        ]
        return options
    ...
    with gr.Blocks() as civitai_interface:
        with gr.Tab(label="Browser", elem_id="browserTab", id="browserTab"):
            with gr.Row(elem_id="searchRow"):
                with gr.Accordion(open=False, elem_id=filterBox):
                    ...
                    with gr.Row(elem_id=component_id):
                        create_json = gr.Checkbox(
                            label=f"Save info after download",
                            value=True,
                            elem_id=toggle1,
                            min_width=171,
                        )
                        ...
                        toggle_date = gr.Checkbox(
                            label="Divide cards by date",
                            value=False,
                            elem_id=toggle3,
                            min_width=142,
                        )
                        only_liked = gr.Checkbox(
                            label="Liked models only",
                            value=False,
                            interactive=show_only_liked,
                            elem_id=toggle4,
                            min_width=163,
                        )
                        hide_installed = gr.Checkbox(
                            label="Hide installed models",
                            value=False,
                            elem_id=toggle5,
                            min_width=170,
                        )
                    ...
                    with gr.Row(elem_id="save_set_box"):
                        save_settings = gr.Button(
                            value="Save settings as default", elem_id="save_set_btn"
                        )
                search_term = gr.Textbox(
                    label="", placeholder="Search CivitAI", elem_id="searchBox"
                )
                refresh = gr.Button(value="", elem_id=refreshbtn, icon="placeholder")
            with gr.Row(elem_id=header):
                with gr.Row(elem_id="pageBox"):
                    get_prev_page = gr.Button(
                        value="Prev page", interactive=False, elem_id="pageBtn1"
                    )
                    ...
                    get_next_page = gr.Button(
                        value="Next page", interactive=False, elem_id="pageBtn2"
                    )
            ...
            with gr.Row(elem_id="select_all_models_container"):
                select_all = gr.Button(
                    value="Select All", elem_id="select_all_models", visible=False
                )
            with gr.Row():
                list_html = gr.HTML(
            ...
            with gr.Row():
                list_models = gr.Dropdown(
                    label="Model:",
                    choices=[],
                    interactive=False,
                    elem_id="quicksettings1",
                    value=None,
                )
                list_versions = gr.Dropdown(
                    label="Version:",
                    choices=[],
                    interactive=False,
                    elem_id="quicksettings0",
                    value=None,
                )
            ...
            with gr.Row():
                with gr.Column(scale=4):
                    install_path = gr.Textbox(
                        label="Download folder:", interactive=False, max_lines=1
                    )
                with gr.Column(scale=2):
                    sub_folder = gr.Dropdown(
                        label="Sub folder:", choices=[], interactive=False, value=None
                    )
            with gr.Row():
                with gr.Column(scale=4):
                    trained_tags = gr.Textbox(
                ...
                with gr.Column(scale=2, elem_id="spanWidth"):
                    ...
                    model_filename = gr.Textbox(
                        label="Model filename:", interactive=False, value=None
                    )
            with gr.Row():
                ...
                delete_model = gr.Button(
                    value="Delete model", interactive=False, visible=False
                )
                download_model = gr.Button(value="Download model", interactive=False)
                subfolder_selected = gr.Dropdown(
                    label="Sub folder for selected files:",
                    choices=[],
                    interactive=False,
                    visible=False,
                    value=None,
                ...
                download_selected = gr.Button(
                    ...
                    interactive=False,
                    visible=False,
                    elem_id="download_all_button",
                )
            with gr.Row():
                cancel_all_model = gr.Button(
                    value="Cancel all downloads", interactive=False, visible=False
                )
                cancel_model = gr.Button(
                    value="Cancel current download", interactive=False, visible=False
                )
            with gr.Row():
        ...
        with gr.Tab(label="Update Models", id="updateTab"):
            with gr.Row():
                selected_tags = gr.CheckboxGroup(
                    elem_id="selected_tags",
                    label="Selected content types:",
                    choices=scan_choices,
                )
            with gr.Row(elem_id="civitai_update_toggles"):
                overwrite_toggle = gr.Checkbox(
                    elem_id="overwrite_toggle",
                    label="Overwrite any existing files. (previews, HTMLs, tags, descriptions)",
                    value=True,
                    min_width=300,
                )
                skip_hash_toggle = gr.Checkbox(
                    elem_id="skip_hash_toggle",
                    label="One-Time Hash Generation for externally downloaded models.",
                    value=True,
                    min_width=300,
                )
                do_html_gen = gr.Checkbox(
                    elem_id="do_html_gen",
                    label="Save HTML file for each model when updating info & tags (increases process time).",
                    value=False,
                    min_width=300,
                )
            with gr.Row():
                save_all_tags = gr.Button(
                    value="Update model info & tags", interactive=True, visible=True
                )
                cancel_all_tags = gr.Button(
                    value="Cancel updating model info & tags",
                    interactive=False,
                    visible=False,
                )
            with gr.Row():
            ...
            with gr.Row():
                update_preview = gr.Button(
                    value="Update model preview", interactive=True, visible=True
                )
                cancel_update_preview = gr.Button(
                    value="Cancel updating model previews",
                    interactive=False,
                    visible=False,
                )
            with gr.Row():
            ...
            with gr.Row():
                ver_search = gr.Button(
                    value="Scan for available updates", interactive=True, visible=True
                )
                cancel_ver_search = gr.Button(
                    value="Cancel updates scan", interactive=False, visible=False
                )
                load_to_browser = gr.Button(
                    value="Load outdated models to browser",
                    interactive=False,
                    visible=False,
                )
            with gr.Row():
            ...
            with gr.Row():
                load_installed = gr.Button(
                    value="Load all installed models", interactive=True, visible=True
                )
                cancel_installed = gr.Button(
                    value="Cancel loading models", interactive=False, visible=False
                )
                load_to_browser_installed = gr.Button(
                    value="Load installed models to browser",
                    interactive=False,
                    visible=False,
                )
            with gr.Row():
                installed_progress = gr.HTML(
                    value='<div style="min-height: 0px;"></div>'
                )
            with gr.Row():
                organize_models = gr.Button(
                    value="Organize model files", interactive=True, visible=False
                )  # Organize models hidden until implemented
                cancel_organize = gr.Button(
                    value="Cancel loading models", interactive=False, visible=False
                )
            with gr.Row():
                organize_progress = gr.HTML(
                    value='<div style="min-height: 0px;"></div>'
                )
        with gr.Tab(label="Download Queue", id="queueTab"):
            ...
            def get_style(size, left_border):
                return (
                    ...
                    + "border-bottom: 1px solid var(--border-color-primary);padding: 5px 10px 5px 10px;width: 0;"
                )

            download_manager_html = gr.HTML(
                elem_id="civitai_dl_list",
                value=f"""
        ...
        def format_custom_subfolders():
            ...
            result = separator.join(
                [f"{key}{separator}{value}" for key, value in data.items()]
            )
            return result
        ...
        gr.Textbox(
            elem_id="custom_subfolders_list",
            visible=False,
            value=format_custom_subfolders(),
        )
        ...
        civitai_text2img_input = gr.Textbox(
            elem_id="civitai_text2img_input", visible=False
        )
        ...
        list_html_input.change(
            fn=None,
            inputs=hide_installed,
            _js="(toggleValue) => hideInstalled(toggleValue)",
        )
        hide_installed.input(
            fn=None,
            inputs=hide_installed,
            _js="(toggleValue) => hideInstalled(toggleValue)",
        )

        civitai_text2img_output.change(
            fn=None,
            inputs=civitai_text2img_output,
            _js="(genInfo) => genInfo_to_txt2img(genInfo)",
        )
        ...
        model_preview_html_input.change(
            fn=None,
            inputs=model_preview_html_input,
            _js="(html_input) => inputHTMLPreviewContent(html_input)",
        )
        ...
        queue_html_input.change(
            fn=HTMLChange, inputs=[queue_html_input], outputs=download_manager_html
        )
        list_html_input.change(
            fn=HTMLChange, inputs=[list_html_input], outputs=list_html
        )
        preview_html_input.change(
            fn=HTMLChange, inputs=[preview_html_input], outputs=preview_html
        )
        ...
        civitai_text2img_input.change(
            fn=txt2img_output,
            inputs=civitai_text2img_input,
            outputs=civitai_text2img_output,
        )
        ...
        def update_models_dropdown(input):
            if not gl.json_data:
                return (
                    gr.Dropdown.update(
                        value=None, choices=[], interactive=False
                    ),  # List models
                    gr.Dropdown.update(
                        value=None, choices=[], interactive=False
                    ),  # List version
        ...
        sub_folder.select(
            fn=select_subfolder, inputs=[sub_folder], outputs=[install_path]
        )
        ...
        save_images.click(
            ...
            inputs=[preview_html_input, model_filename, install_path, sub_folder],
            outputs=[],
        )
        ...
        refresh_inputs = [
            empty if item == page_slider else item for item in page_inputs
        ]
        ...
        ver_start.change(
            ...
            outputs=[version_progress, ver_finish],
        )
        ...
        installed_start.change(
            fn=_file.file_scan,
            inputs=file_scan_inputs,
            outputs=[installed_progress, installed_finish],
        )
        ...
        tag_start.change(
            fn=_file.file_scan,
            inputs=file_scan_inputs,
            outputs=[tag_progress, tag_finish],
        )
        ...
        preview_start.change(
            fn=_file.file_scan,
            inputs=file_scan_inputs,
            outputs=[preview_progress, preview_finish],
        )
        ...
        create_subfolder.change(
            fn=_file.updateSubfolder, inputs=create_subfolder, outputs=[]
        )
...
def on_ui_settings():
    ...
    if not (
        hasattr(shared.OptionInfo, "info")
        and callable(getattr(shared.OptionInfo, "info"))
    ):
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Disable this option if you're experiencing any issues with downloads or if you want to use a proxy."
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Useful for users who use PortMaster or other software that controls the DNS"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "creates an api_info.json file when saving any model info with all the API data of the model"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "You can create your own API key in your CivitAI account settings, this required for some downloads, Requires UI reload"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Uses the matching local HTML file when pressing CivitAI button on model cards in txt2img and img2img"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Keeps the page navigation always visible at the top, Requires UI reload"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Disable this option if you're experiencing high CPU usage during video/gif playback"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Turns individual prompts from an example image into a button to send it to txt2img"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "This saves the models description to the description field on model cards"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Only works with proxies that support HTTPS, turn Aria2 off for proxy downloads"
        ),
    )
    ...
    shared.opts.add_option(
        ...
        shared.OptionInfo(
            ...
            **({"category_id": cat_id} if ver_bool else {}),
            ...
            "Not recommended for security, may be required if you do not have the correct CA Bundle available"
        ),
    )
